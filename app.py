...
with tab1:
    ...
    if st.button("CALCULATE SAFETY SCORE", key="t1_calc"):
        ...
        with rc2:
            # Build next-level hint outside f-string (backslashes not allowed inside f-string {})
            if next_l:
                next_hint = (
                    f"<div style='font-size:11px;color:#1e3a5f;margin-top:6px;'>"
                    f"{next_l['points_needed']} pts to {next_l['name']}"
                    f"</div>"
                )
            else:
                next_hint = ""
